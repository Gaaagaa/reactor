...
BOOL CXftpClientDlg::OnInitDialog()
{
    ...
    m_wndListFiles.InsertColumn(nCol++, TEXT("文件名称"), LVCFMT_LEFT  , 150);
    m_wndListFiles.InsertColumn(nCol++, TEXT("文件大小"), LVCFMT_CENTER, 80 );
    m_wndListFiles.InsertColumn(nCol++, TEXT("下载进度"), LVCFMT_CENTER, 120);
    m_wndListFiles.InsertColumn(nCol++, TEXT("下载速度"), LVCFMT_CENTER, 80 );
...
{
    XVERIFY(__obser_type::register_mkey(x_ftp_client_t::CMID_WCLI_FLIST, &CXftpClientDlg::on_msg_flist));
...
BEGIN_MESSAGE_MAP(CXftpClientDlg, CDialogEx)
    ...
    ON_BN_CLICKED(IDC_BTN_DOWNLOAD, &CXftpClientDlg::OnBnClickedBtnDownload)
...
LRESULT CXftpClientDlg::OnKickIdle(WPARAM wParam, LPARAM lParam)
{
    ...
    BOOL bSelItem = (m_wndListFiles.GetSelectedCount() > 0) && bLogin;
    ...
    GetDlgItem(IDC_BTN_DOWNLOAD )->EnableWindow(bSelItem);
    GetDlgItem(IDC_BTN_DOWNSTOP )->EnableWindow(bSelItem);
...
/**********************************************************/
/**
 * @brief WM_COMMAND[BN_CLICKED, IDC_BTN_DOWNLOAD] message handler.
 */
void CXftpClientDlg::OnBnClickedBtnDownload()
{
    if (!m_xftp_client.is_startup())
    {
        return;
