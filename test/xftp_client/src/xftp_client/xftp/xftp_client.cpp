...
x_int32_t x_ftp_client_t::iocmd_flist(x_uint16_t xut_seqn, x_uchar_t * xct_dptr, x_uint32_t xut_size)
{
    do 
    {
        ...
        if (!j_reader.parse((x_char_t *)xct_dptr, (x_char_t *)xct_dptr + xut_size, j_flist, false))
        {
            break;
        }
        ...
        for (x_uint32_t xut_iter = 0; xut_iter < j_flist.size(); ++xut_iter)
        {
            ...
            {
                xlst_file.push_back(std::make_pair(j_file["file"].asString(), j_file["size"].asUInt()));
            }
        }
        ...
        x_msg_handler_t::instance().post_msg(EM_EST_CLIENT, CMID_WCLI_FLIST, X_NULL, 0, X_NULL);
